...
def get_solution_1(input):
    ...
    for move, move_val in input:
        ...
        if move == "F":
            ...
            ship_pos.x, ship_pos.y = ship_pos.x + move_val*math.sin(rad), ship_pos.y + move_val * math.cos(rad)
...
def get_solution_2(input):
    ...
    for move, move_val in input:
        ...
        if move == "W":
            waypoint_pos.x -= move_val
        if move == "L":
            rad = math.atan2(waypoint_pos.y, waypoint_pos.x) + math.radians(move_val)
            dist = math.sqrt(waypoint_pos.y**2 + waypoint_pos.x**2)
            waypoint_pos.x,  waypoint_pos.y  = dist * math.cos(rad) ,  dist * math.sin(rad)
        if move == "R":
            rad = math.atan2(waypoint_pos.y, waypoint_pos.x) - math.radians(move_val)
            dist = math.sqrt( waypoint_pos.y**2 + waypoint_pos.x **2  )
            waypoint_pos.x,  waypoint_pos.y  = dist * math.cos(rad) ,  dist * math.sin(rad)
        if move == "F":
            ship_pos.x, ship_pos.y = ship_pos.x + move_val * waypoint_pos.x ,  ship_pos.y + move_val * waypoint_pos.y
    return ship_pos.getManhattanDistance()
